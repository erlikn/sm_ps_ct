...
import Data_IO.data_input_deconv as data_input
...
#TEST###        prevTestSumLoss = 99999
		...
		for step in xrange(epochNumber, modelParams['maxSteps']):
			...
			_, lossValue, l2regValue = sess.run([opTrain, loss, output_res['l2reg']])
			#_, lossValue, l2regValue, out = sess.run([opTrain, loss, output_res['l2reg'], output_res])
			#cv2.imshow('out', out['deconv'][0,:,:])
			#cv2.waitKey(0)
...
def main(argv=None):  # pylint: disable=unused-argumDt
	...
	#	return
	modelName = '181114rg0_deconv'#argv[1]
	epochNumber = 0#int(argv[2])
