...
def inference(images, **kwargs): #batchSize=None, phase='train', outLayer=[13,13], existingParams=[]
    ...
    if kwargs.get('batchNorm'):
        fireOut1 = model_base.batch_norm('batchnorm1', fireOut1, dtype)
    ...
    if kwargs.get('batchNorm'):
        fireOut1 = model_base.batch_norm('batchnorm2', fireOut1, dtype)
    ###### Pooling1 2x2 wit stride 2
    fireOut1 = tf.nn.max_pool(fireOut1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
    ...
    if kwargs.get('batchNorm'):
        fireOut2 = model_base.batch_norm('batchnorm3', fireOut2, dtype)
    ###### Pooling1 2x2 wit stride 2
    ...
    if kwargs.get('batchNorm'):
        fireOut2 = model_base.batch_norm('batchnorm4', fireOut2, dtype)
    ...
    if kwargs.get('batchNorm'):
        fireOut3 = model_base.batch_norm('batchnorm5', fireOut3, dtype)
    # Pooling2 2x2 wit stride 2
    ...
    if kwargs.get('batchNorm'):
        fireOut3 = model_base.batch_norm('batchnorm6', fireOut3, dtype)
    ###### Pooling2 2x2 wit stride 2
    fireOut3 = tf.nn.max_pool(fireOut3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
    ...
    if kwargs.get('batchNorm'):
        fireOut4 = model_base.batch_norm('batchnorm7', fireOut4, dtype)
    # Pooling2 2x2 wit stride 2
    #fireOut1 = tf.nn.max_pool(fireOut1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
    ...
    if kwargs.get('batchNorm'):
        fireOut4 = model_base.batch_norm('batchnorm8', fireOut4, dtype)
    # CONCAT
    ...
    fireOut1 = tf.reshape(fireOut1, [batchSize, -1])
    prevExpandDim = int(fireOut.get_shape()[1])
    ############# FC1 layer with 1024 outputs
    fireOut1, prevExpandDim = model_base.fc_fire_module('fc1', fireOut1, prevExpandDim,
                                                       {'fc': modelShape[8]},
                                                       wd, **kwargs)
    # calc batch norm FC1
    if kwargs.get('batchNorm'):
    ...
    # calc batch norm FC1
    if kwargs.get('batchNorm'):
        fireOut1 = model_base.batch_norm('batchnorm10', fireOut1, dtype)
    ...
    # fireout1 is [16] x [192] 
    # fireout1 should be [16] x [6] x [32] x [1] => now normalize for each batch and each row
    ...
    #fireOut1 = tf.reshape(fireOut1, [kwargs.get('activeBatchSize')*kwargs.get('logicalOutputSize'), np.int32(kwargs.get('networkOutputSize')/kwargs.get#('logicalOutputSize'))])
    ...
    #fireOut1 = tf.nn.softmax(fireOut)

    #### NOW CONVERT IT TO Correct format
    # kwargs.get('networkOutputSize')/kwargs.get('logicalOutputSize') = kwargs.get('classificationModel')['binSize']
    
    #fireOut1 = tf.reshape(fireOut1, [kwargs.get('activeBatchSize'), 
