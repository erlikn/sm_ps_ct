...
def inference(images, **kwargs): #batchSize=None, phase='train', outLayer=[13,13], existingParams=[]
    ...
    ############# CONV3
    fireOut, prevExpandDim = model_base.conv_fire_inception_module('conv3', pool, prevExpandDim,
   ...
   # calc batch norm CONV4
    ...
    prevExpandDim = (kwargs.get('pngRows')//(2*2*2))*(kwargs.get('pngCols')//(2*2*2))*prevExpandDim
